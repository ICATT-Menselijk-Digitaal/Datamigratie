...
public class ZakenGenerator(IConfiguration configuration, ILogger<ZakenGenerator> logger)
{
    ...
    public async Task Generate()
    {
        ...
        logger.LogInformation("Zaken per type: {zakenPerType}", _zakenPerType);
        ...
        await foreach (var catalogus in GetCatalogusData.GetCatalogi())
        {
            foreach (var zaaktype in catalogus.Zaaktypen)
            {
                ...
                logger.LogInformation("Generating {zakenPerType} zaken for: {zaaktype.Omschrijving} ({zaaktype.Domein})", _zakenPerType, zaaktype.Omschrijving, zaaktype.Domein);

                for (var i = 0; i < _zakenPerType; i++)
                {
