...
public sealed class ZaakFaker
{
    ...
    {
        _zaakCounter++;
        var zaakFunctioneleIdentificatie = $"{zaaktype.Identificatie ?? zaaktype.Omschrijving}-{_zaakCounter:D6}-TEST";
    ...
    private string GenerateOmschrijving(Zaaktype zaaktype)
    {
        ...
        return $"{_faker.PickRandom(templates)}-TEST";
    }
    ...
    private IReadOnlyList<ZaakDocument> GenerateDocumenten(Zaaktype zaaktype, DateTime startDatum, string zaakFunctioneleIdentificatie)
    {
        ...
        return verplichteDocs.Concat(optioneleDocs)
            .Select((d, i) =>
            {
                var docId = $"DOC-{_zaakCounter:D6}-{i + 1:D3}-TEST";
                ...
                return new ZaakDocument
                {
                    ...
                    Titel = $"{docNaam} - {_faker.Lorem.Words(2).Aggregate((a, b) => $"{a} {b}")}-TEST",
                    Kenmerk = _faker.Random.Bool(0.3f) ? $"KNM-{_faker.Random.AlphaNumeric(8).ToUpperInvariant()}-TEST" : null,
    ...
    private IReadOnlyList<Besluit>? GenerateBesluiten(Zaaktype zaaktype, bool isAfgerond, DateOnly streefdatum)
    {
        ...
        return zaaktype.Besluittypen?.Select(besluittype =>
        {
            ...
            return new Besluit
            {
                FunctioneleIdentificatie = $"BESL-{_zaakCounter:D6}-TEST",
