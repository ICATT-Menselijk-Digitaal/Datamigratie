...
const fillRandom = () => {
  mappingsModel.value = sourceResultaattypeItems.value.map((sourceItem) => ({
    sourceId: sourceItem.id,
    targetId:
      targetResultaattypeItems.value[
        Math.floor(Math.random() * targetResultaattypeItems.value.length)
      ]?.id ?? null
  }));
};
