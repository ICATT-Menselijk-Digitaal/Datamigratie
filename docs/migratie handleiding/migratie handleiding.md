...
## Achterliggende migratiedetails
De migratie wordt uitgevoerd op basis van het mappingsdocument dat is opgesteld door Michèl de Hoog en Giuseppe Rolesu. Dit document beschrijft welke e-Suite-velden naar welke OZ-velden overgezet moeten worden. Deze mapping is geaccordeerd door de werkgroep informatiebeheer en verschillende gemeenten.

Velden die tijdens de ontwikkeling alsnog foutief gemapt bleken te zijn (bijvoorbeeld door onderliggende OZ-logica/restricties of een mismatch tussen datatypes) zijn overgeslagen of op een andere manier opgelost. De documentatie daarvan is te vinden in het [mappingsoverzicht](https://miro.com/app/board/uXjVJoXZzDY=/?share_link_id=16596598973). In dit overzicht zijn de velden opgedeeld op basis van bij welke OpenZaak API-call ze horen. De documentatie van de verschillende ZGW API's is te vinden in de [OpenZaak ReDoc](https://open-zaak.readthedocs.io/en/stable/api/index.html).
...
- **Informatieobjecttype voor gegenereerde PDF:** Aan elke zaak wordt in OZ een gegenereerde PDF toegevoegd. Deze PDF bevat de volgende informatie:
    ...
    - Alle velden die in eerste instantie "gemapt" waren, maar waarvan tijdens de ontwikkeling bleek dat deze mapping niet mogelijk was.
...
**Let op:** het is *niet* mogelijk om verschillende zaaktypes tegelijk te migreren, ook niet vanaf meerdere accounts.
...
3. **Eén zaak**  
    Met deze optie is het mogelijk om één enkele zaak te migreren op basis van het zaaknummer van de e-Suite.
